...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		if ($query->num_rows() === 1)
		{
			...
			$what_happen = $this->db->get_where('users', array('username' => $identity), 1, 0);

			if(!empty($what_happen)){
				if ($what_happen->is_active == 0)
				{ 
					
					$this->trigger_events('post_login_unsuccessful');
					$this->set_error('login_unsuccessful_not_active');
					$this->set_error_kod("2");
	...
	public function get_users_groups($id = FALSE)
	{
		...
		return $this->db->select($this->tables['users_groups'].'.'.$this->join['groups'].' as id, '.$this->tables['groups'].'.name')
	 ...
	 * @author Ben Edmunds
	 */
	public function set_error_kod($error_kod){
		$this->_errors_kod = $error_kod;
		return $error_kod;
	}
	 ...
	 * @author Ben Edmunds
	 */
	public function get_error_kod(){
		return $this->_errors_kod;
	}
	 ...
	 * @return bool
	 **/
	protected function _diactivated_temporary_user($id)
	{
		$this->db->update($this->tables['users'], ['is_active' => 0] , ['id' => $id]);
	}
}
